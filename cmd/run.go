...
func init() {
	...
	runCmd.Flags().BoolVar(&runCmdFlags.dryRun, "dry-run",
    false, "dry run")
	runCmd.Flags().StringVar(&runCmdFlags.tagRegexp, "tag-regexp",
    ".*", "tag regexp")
	runCmd.Flags().StringArrayVarP(&runCmdFlags.statuses, "status", "s",
    []string{}, "List of status to succeed")
}
